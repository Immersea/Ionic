 ...
 */
 export class BuhlmannDecoDP4 {
//    private static Body body; //the body we are currently working on
    private static currentDive; //the dive we are currently working on
    
//    private static double PH2O;	// Water vapor Pressure
    private static surfacePressure;

    private static Currently_Max_Allowed_Gradient_Factor;
    
//    private static ObservableList<DiveDecoSegment> resultingDecoPlan;


//    private Fraction_Oxygen = new Array[10];
//    private int Number_of_Changes;
//    private int Mix_Number;
//    private Depth_Change;
...
//    private static double Last_Run_Time, Run_Time,Stop_Time;
    ...
    private PO2LO = new Array[11];
    private PO2HI = new Array[11];
    private LIMSLP = new Array[11];
    private LIMINT = new Array[11];

    private static  CNStoxicityPercentage;
    private static OTUbuildup;


    // N2 compartment pressures
    private static nitrogenCompartmentPressure = new Array[16]; //PN2
    
    private static nitrogenHalfTimes = new Array[16];

    // N2 Half-time constants
    private static kN2 = new Array[16];

    // He compartment pressures
    private static heliumCompartmentPressure = new Array[16]; //PHe

    private static heliumHalfTimes = new Array[16];
    
    // He Half-time constants
    private static kHe = new Array[16];

    // surfacing N2 M-values
//	private N2_M0;
//	private final N2_deltaM = {1.7928, 1.5352, 1.3847, 1.2780, 1.2306, 1.1857, 1.1504, 1.1223, 1.0999, 1.0844, 1.0731, 1.0635, 1.0552, 1.0478, 1.0414, 1.0359};

    // surfacing He M-values
//	private He_M0;
//	private final He_deltaM = {2.3557, 2.0964, 1.74, 1.5321, 1.3845, 1.3189, 1.2568, 1.2079, 1.1692, 1.1419, 1.1232, 1.1115, 1.1022, 1.0963, 1.0904, 1.085, 1.0791};

    private static N2_a = new Array[16];
    private static N2_a_Bar = new Array[16];
    private static N2_b = new Array[16];
    private static He_a = new Array[16];
    private static He_a_Bar = new Array[16];
    private static He_b = new Array[16];
    
    private static buhlmannDeco = new BuhlmannDecoDP4.BuhlmannDeco();
...
//System.out.println("Constant depth input at " + diveSegment.startDepth + ". gasID: " + diveSegment.gasID + " Duration: " + diveSegment.duration);
...
//System.out.println("stoppedAfterAscent || !ascentDone");
                    ...
                    if(diveSegment.userDefined && firstShallowerUserInput)
                    {
//System.out.println("Setting back the duration to " + originalSegmentDuration);
                    ...
                    if(Double.parseDouble(decoSegment.getStartRunTime()) % Settings.Minimum_Deco_Stop_Time > 0)
                    {
...
//System.out.println("After projected ascent, deco is still needed between " + diveSegment.startDepth + " -> " + diveSegment.endDepth);                        
                    ...
                    else
                    {
                        ...
                        { 
                            currentDive.offgassingStartsAtDepth = CALC_START_OF_DECO_ZONE(diveSegment.startDepth);
//System.out.println("DiveSegment.ASCENT: Saving offgassing startdepth = " + currentDive.offgassingStartsAtDepth + " FROM: " + diveSegment.startDepth);                            
                        }
...
//        Body.saveSurfacingValuesForDive(currentDive.missionDiveNumber, nitrogenCompartmentPressure, heliumCompartmentPressure, CNStoxicityPercentage, OTUbuildup);
    ...
    private static double CALC_DECO_CEILING()
    {
        ...
        // LOCAL ARRAYS
        //===============================================================================
        Compartment_Deco_Ceiling = new Array[16];
        //===============================================================================
...
*/        
        ...
        ===============================================================================*/
        //Debug, diveSegments
/*        System.out.println("At " + starting_depth + "m :");
        for(int i=0; i<16; i++)
        {
            System.out.println("Cmp " + i + ": " + body.nitrogenCompartmentPressure[i]);
        }  */
...
//System.out.println("In decompress(...) - Stop depth, after projected ascent: " + Deco_Stop_Depth);        
        ...
        if(Settings.lastStopDoubleInterval && Deco_Stop_Depth <= (2 * Settings.decoStopInterval))
        {
...
//            decoSegment.setGradientFactor(Util.roundToOneDecimal(Gradient_Factor_Current_Stop*100));
            ...
            if(firstStop)
            {
                ...
                {
                    adjustedStopDuration = Math.round((stopDuration/Settings.Minimum_Deco_Stop_Time) + 0.5) * Settings.Minimum_Deco_Stop_Time;
                }
                //we don't include the ascent-time in the first deco stop duration
...
//System.out.println("adding stop, " + stopSegment.duration + " minutes. " + stopSegment.startDepth + " to " + stopSegment.endDepth);            
            ...
            ===============================================================================*/
/*            if(Math.round(Settings.Minimum_Deco_Stop_Time) == Settings.Minimum_Deco_Stop_Time)
            {
...
//            Last_Run_Time = Run_Time;
        ...
        if(ending_depth == 0)
        {
            ...
            decoSegment.setStartRunTime(Math.round((currentDive.currentRunTime/Settings.Minimum_Deco_Stop_Time) + 0.5) * Settings.Minimum_Deco_Stop_Time);
            decoSegment.setUserDefined(true);
            currentDive.resultingDecoPlan.add(decoSegment);
...
/*		ELSE
    ...
    private static double PROJECTED_ASCENT(double Starting_Depth, double Rate, double Deco_Stop_Depth, double Step_Size)
    {
        ...
        /*===============================================================================
         LOCAL ARRAYS
        ===============================================================================*/
        Initial_Helium_Pressure = new Array[16];
        Initial_Nitrogen_Pressure = new Array[16];
        Temp_Gas_Loading = new Array[16];
        Allowable_Gas_Loading = new Array[16];
        /*===============================================================================
...
//if(Depth == 50 && i == 0) System.out.println("GAS_LOADINGS_CONSTANT_DEPTH. före: " + Initial_Nitrogen_Pressure + " efter: " + nitrogenCompartmentPressure[i] + " Inspired_Nitrogen_Pressure: " + Inspired_Nitrogen_Pressure + " time constant: " + kN2[i]);
    }
}
    ...
    public static void performSurfaceInterval(nitrogenTissuePressures, heliumTissuePressures, int minutes)
    {
    ...
    public static getNitrogenCompartmentPressures()
    {
        return nitrogenCompartmentPressure;
    }
    public static getHeliumCompartmentPressures()
    {
...
//System.out.println("PO2: " + PO2 + " at depth: " + Depth + " Ambient Pressure ATA: " + Ambient_Pressure_ATA + " Current oxygen fraction: " + currentDive.getCurrentOxygenFraction());
    ...
    {        
        ...
        temp_nitrogenCompartments = nitrogenCompartmentPressure.clone();
        temp_heliumCompartments = heliumCompartmentPressure.clone();
...
//System.out.print("Now at stop depth: " + Deco_Stop_Depth + " runtime: " + Last_Run_Time);        
        //Här rundar vi upp så run_time slutar på en hel minut (eller vad minimum_deco_stop_time nu är) när vi avslutar detta dekostoppet.
        //Round_Up_Operation = Math.round((Last_Run_Time/Settings.Minimum_Deco_Stop_Time) + 0.5) * Settings.Minimum_Deco_Stop_Time;
        Round_Up_Operation = Math.floor(Last_Run_Time) + Settings.Minimum_Deco_Stop_Time; //ändrade till detta istället för raden ovan i.o.m. v4.1.1.
...
/*            temp2_nitrogenCompartments = nitrogenCompartmentPressure.clone();
            temp2_heliumCompartments = heliumCompartmentPressure.clone();
