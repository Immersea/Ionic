...
export class MapDataDivingSchool {
  id: string;
  displayName: string;
  photoURL: string;
  coverURL: string;
  position: Position;
  divingCourses: DivingCourse[]; //ids
...
export class DivingSchool {
  ...
  facebook: string;
  instagram: string;
  divingCourses: DivingCourse[]; //list of dive courses
  users: {
  ...
  constructor(data?) {
    ...
    this.divingCourses = [];
    if (data && data.divingCourses) {
      this.divingCourses = data.divingCourses;
    }
    this.users = {};
    if (data && data.users) {
