...
export class MapDataDivingCenter {
  id: string;
  displayName: string;
  photoURL: string;
  coverURL: string;
...
export class DivingCenter {
  ...
  facebook: string;
  instagram: string;
  diveSites: string[]; //list of center dive sites
  users: {
  ...
  constructor(data?) {
    ...
    this.diveSites = [];
    if (data && data.diveSites) {
      this.diveSites = data.diveSites;
    }
    this.users = {};
    if (data && data.users) {
