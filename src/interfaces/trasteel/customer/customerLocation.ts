...
export class CustomerLocation {
  ...
  electrodesData: CustomerLocationElectrodesData[];
  type: string;
  ...
  constructor(data?) {
    ...
    this.plant_age =
      data && data.plant_age
        ? !isNaN(toNumber(data.plant_age))
          ? toNumber(data.plant_age)
          : data.plant_age
        : null;
    ...
    this.workforce_size =
      data && data.workforce_size
        ? !isNaN(toNumber(data.workforce_size))
          ? toNumber(data.workforce_size)
          : data.workforce_size
        : null;
    this.ISO_14001 =
      data && has(data, "ISO_14001")
        ? isBoolean(data.ISO_14001)
          ? data.ISO_14001
          : isString(data.ISO_14001)
            ? data.ISO_14001
            : null
        : null;
    this.ISO_50001 =
      data && has(data, "ISO_50001")
        ? isBoolean(data.ISO_50001)
          ? data.ISO_50001
          : isString(data.ISO_50001)
            ? data.ISO_50001
            : null
        : null;
    this.responsible_steel_certification =
      data && has(data, "responsible_steel_certification")
        ? isBoolean(data.responsible_steel_certification)
          ? data.responsible_steel_certification
          : isString(data.responsible_steel_certification)
            ? data.responsible_steel_certification
            : null
        : null;
...
export class CustomerConditions {
  EAF: CustomerConditionEAF[];
  LF: CustomerConditionLF[];
  CCM: CustomerConditionCCM[];
...
export class CustomerConditionEAF {
  date: string;
  capacity: number;
  n_shells: number;
  shell_diam: number;
  ...
  o2_cons: number;
  n_burners: number;
  foaming_slag: boolean;
  porous_plugs: number;
  stirring_gas: string;
  alloys: string;
  current_lifetime: number;
  target_lifetime: number;
  working_lining_weight: number;
  weak_area: string;
...
export class CustomerConditionLF {
  date: string;
...
export class CustomerConditionCCM {
  date: string;
...
export class CustomerProductionData {
  ...
  constructor(data?) {
    ...
    if (
      ...
      has(data.crude_steel_production, "nominal")
    ) {
    ...
    if (
      ...
      has(data.BOF_steel_production, "nominal")
    ) {
    ...
    if (
      ...
      has(data.EAF_steel_production, "nominal")
    ) {
    ...
    if (
      ...
      has(data.OHF_steel_production, "nominal")
    ) {
    ...
    if (data && data.iron_production && has(data.iron_production, "nominal")) {
    ...
    if (data && data.BF_production && has(data.BF_production, "nominal")) {
    ...
    if (data && data.DRI_production && has(data.DRI_production, "nominal")) {
...
export class CustomerLocationElectrodesData {
  application: string;
  dia_in_mm: number;
  length_in_mm: number;
  ...
  constructor(data?) {
    ...
    this.dust_groove =
      data && isBoolean(data.dust_groove) ? data.dust_groove : null;
    this.pitch_plug =
      data && isBoolean(data.pitch_plug) ? data.pitch_plug : null;
    this.preset = data && isBoolean(data.preset) ? data.preset : null;
  }
}
